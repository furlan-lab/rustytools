...
pub fn pcha(
    ...
    // replace these two dot calls by a single GEMM each:
    let mut sst_mat = Array2::<f64>::zeros((noc, noc));
    general_mat_mul(1.0, &s, &s.t(), 0.0, &mut sst_mat);
    let mut ct_x_t_xc = Array2::<f64>::zeros((noc, noc));
    general_mat_mul(1.0, &xc.t(), &xc, 0.0, &mut ct_x_t_xc);

    let mut sse = {
        // let term1 = xc.t().dot(&x_u);
        // sst - 2.0 * (&term1 * &s).sum() + (&ct_x_t_xc * &sst_mat).sum()
        let mut term1 = Array2::<f64>::zeros((noc,nu));
        general_mat_mul(1.0, &xc.t(), &x_u, 0.0, &mut term1);
        sst - 2.0 * (&term1 * &s).sum() + (&ct_x_t_xc * &sst_mat).sum()
    };
...
fn s_update(
    ...
    let mut sst_mat = Array2::<f64>::zeros((noc,noc));
    general_mat_mul(1.0, &s_cur, &s_cur.t(), 0.0, &mut sst_mat);
    ...
    for _ in 0..niter {
        ...
        while !stop {
            ...
            // fused GEMM + parallel simplex
            // let mut candidate = s_old.clone();
            // general_mat_mul(1.0, &grad, &candidate, 1.0, &mut candidate);
            // project_simplex_columns(&mut candidate);
            // s_cur = candidate;
            // general_mat_mul(1.0, &s_cur, &s_cur.t(), 0.0, &mut sst_mat);
            // mutable borror error

            //     // gradient step
            // let mut candidate = &s_old - &(grad.to_owned() * mu_s);

            // // project each column back to the simplex (parallelised)
            // project_simplex_columns(&mut candidate);

            // // recompute SSE etc. (unchanged – you already handle this later)
            // s_cur = candidate;
            // general_mat_mul(1.0, &s_cur, &s_cur.t(), 0.0, &mut sst_mat);
            let mut candidate = &s_old - &(grad.to_owned() * mu_s);
            project_simplex_columns(&mut candidate);
...
fn c_update(
    ...
    for _ in 0..niter {
        ...
        // replace that two‐step dot by one fused GEMM + one t‐GEMM:
        // let mut tmp = Array2::<f64>::zeros((j,noc));
        // general_mat_mul(1.0, &xc, &sst_mat, 0.0, &mut tmp);
        // let mut grad = Array2::<f64>::zeros((j,noc));
        // general_mat_mul(1.0, &x_i.t(), &tmp, 0.0, &mut grad);
        // rows of xc  (= p), not j
        let mut tmp = Array2::<f64>::zeros((xc.nrows(), noc));   // 60 × 3 here
        general_mat_mul(1.0, &xc, &sst_mat, 0.0, &mut tmp);

        // now tmp is 60 × 3; the next line multiplies x_iᵀ (300 × 60) · tmp (60 × 3)
        // giving grad of size j × k = 300 × 3 – exactly what we need
        let mut grad = Array2::<f64>::zeros((j, noc));
        general_mat_mul(1.0, &x_i.t(), &tmp, 0.0, &mut grad);
        grad -= &x_t_x_s_t;
        grad.mapv_inplace(|v| v / sst);
        if delta != 0.0 {
        ...
        while !stop {
            ...
            // fused step
            // general_mat_mul(1.0, &grad, &c_old, 1.0, &mut c_cur);
            ...
            // }
            let mut c_cur = &c_old - &(grad.to_owned() * mu_c);
            c_cur.mapv_inplace(|v| v.max(0.0));
            for mut col in c_cur.columns_mut() {
                let s = col.sum().max(f64::EPSILON);
