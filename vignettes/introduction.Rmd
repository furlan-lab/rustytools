...
query <- charToRaw("ACCGTGGAT")
reference <- charToRaw("AAAAACCGTTGAT")

# Perform semiglobal alignment
score <- align(query, reference, atype = "semiglobal")
score
```
...
* **FASTA reading** – random access to large genome files
  See the [Fasta vignette](articles/getConsensus.html)
* **MAGIC** – imputation of scRNA-seq using diffusion maps
  See the [MAGIC vignette](articles/magic.html)
* **PCHA** – archetypal decomposition of matrices
  See the [PCHA vignette](articles/pcha.html)
