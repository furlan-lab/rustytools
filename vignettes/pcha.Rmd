...
ks <- 3:8
microbenchmark::microbenchmark(sse <- sapply(ks, function(k) pcha(X, k)$sse), times = 5)
```
...
We can compare the performance of our rust implementation to the paretoTI package (note that this uses python and requires reticulate).  We see a modest gain in performance using rust
```{r}
suppressPackageStartupMessages({library(ParetoTI)})
...
noc<-5
res <- fit_pch(X, noc = noc, maxiter = 750, normalise_var = F, conv_crit = 1e-6)
data_dim = seq(1, noc - 1)


hull_vol = fit_convhulln(X[data_dim, ], positions = FALSE)
...
archetypes2 = res$XC[data_dim, ]
arch_red = archetypes2 - archetypes2[, noc]
arc_vol = abs(Matrix::det(arch_red[, seq_len(noc - 
                1)])/factorial(noc - 1))
hull_vol = hull_vol$vol
t_ratio = arc_vol/hull_vol
